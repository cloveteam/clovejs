...
function jsonEnabled(route, res) {
...
function applyJsonResult(result, route, res, method) {
  if (res.sent) return;
  if (result === void 0) {
...
async function runPipeline(route, req, res, container, options) {
...
function composeChain(route, req, res, ctx, middlewares) {
  let index = -1;
...
function writeError(err, res, options) {
  if (res.sent) {
...
async function createLoader(rootDir, options = {}) {
...
async function scanProject(options) {
  ...
  for (const file of await walkDir(apiDir)) {
    ...
    if (derived.method !== null && derived.method !== route.method) {
      throw new CloveBootError(
        `Method mismatch: the filename says ${derived.method} but the handler is wrapped in ${route.method.toLowerCase()}(). Make them agree, or drop the method suffix from the filename.`,
        [file.absolute]
      );
    }
    const registered = {
...
var WsRuntime = class {
  ...
  handleUpgrade(req, socket, head) {
    ...
    const route = match ? this.#options.handlers.get(match.route.path) : void 0;
    if (!match || !route) {
  ...
  async #open(socket, raw, route, params) {
...
var CloveApp = class {
  ...
  #options;
  #closed = false;
  constructor(scan, root, logger, sessions, ws, options) {
    this.scan = scan;
...
//# sourceMappingURL=chunk-ZGXC75VM.js.map